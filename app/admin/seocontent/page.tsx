...
export default function SEODashboard() {
  ...
  const fetchSlugs = async () => {
    try {
      const res = await axios.get("/api/seo/list");
      // Assuming structure is { success: true, data: [{ slug: '...' }] }
      setSlugs(res.data.data.map((item: any) => item.slug));
    } catch (error) {
  ...
  // 2. Load Specific SEO Data
  const loadSEO = async (slug: string) => {
    setSelectedSlug(slug);
    setLoadingContent(true);
    try {
      const res = await axios.get(`/api/seo/${slug}/get`);
  ...
  const createNewSlug = async () => {
    ...
    try {
      ...
      setNewSlug("");
      loadSEO(newSlug); // Automatically select the new one
    } catch (error) {
  ...
  // 5. Add FAQ
  const addFAQ = async () => {
    if (!newQuestion || !newAnswer) return toast.error("Fill both fields");
    
    // Optimistic UI update
    const newFaqItem = { question: newQuestion, answer: newAnswer };
    const prevFaqs = [...faqs];
    ...
    try {
      ...
      setFaqs(prevFaqs); // Revert on error
  ...
  // Filter slugs for search
  const filteredSlugs = slugs.filter(s => s.toLowerCase().includes(searchTerm.toLowerCase()));
 return (
          ...
          <div className="flex-1 overflow-y-auto space-y-1 pr-2 custom-scrollbar">
            {loadingList ? (
              ...
              filteredSlugs.map((slug) => (
                ...
                >
                  <FileText size={16} className={selectedSlug === slug ? "text-indigo-500" : "text-slate-400"} />
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden w-full">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm shrink-0 ">
          <h2 className="text-lg font-semibold text-slate-800">
            {selectedSlug ? (
              <>Editing: <span className="text-indigo-600 font-mono bg-indigo-50 px-2 py-1 rounded">{selectedSlug}</span></>
            ) : (
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-slate-50">
          {selectedSlug && !loadingContent && (
            // CHANGED: Removed grid grid-cols-3, used flex-col instead
            <div className="max-w-4xl mx-auto flex flex-col gap-8 pb-20">
              ...
              <div className="w-full bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  ...
                  {/* Add New FAQ - Updated to use Grid for inputs */}
                  <div className="bg-slate-50 p-5 rounded-lg mb-6 border border-slate-200">
                    <h4 className="text-sm font-semibold text-slate-600 mb-3">Add New FAQ</h4>
                  ...
                  <div className="space-y-3">
                    {faqs.length === 0 && <p className="text-center text-slate-400 text-sm py-8">No FAQs added yet.</p>}
                    
                    {faqs.map((faq, i) => (
                      <div key={i} className="group flex items-start justify-between bg-white border border-slate-200 p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                        <div className="pr-8">
                          <h4 className="font-semibold text-slate-800 text-sm mb-1">Q: {faq.question}</h4>
                          <p className="text-slate-600 text-sm leading-relaxed">A: {faq.answer}</p>
                        </div>
                        <button
